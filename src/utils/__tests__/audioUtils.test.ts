...
describe('AudioManager', () => {
  ...
  it('should handle error when stopping audio fails', async () => {
    ...
    (audioManager as any).audioElement = {
    ...
    (audioManager as any).fadeOut = jest.fn().mockResolvedValue(undefined);

    await audioManager.stop();

    expect(console.error).toHaveBeenCalledWith('Error stopping audio:', mockError);
  });
});
