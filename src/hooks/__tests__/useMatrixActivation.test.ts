...
describe("shouldShowMatrixFromSearch", () => {
  it("returns false if no matrix parameter is present", () => {
    ...
    expect(shouldShowMatrixFromSearch(new URLSearchParams("?other=1"))).toBe(
      false,
    );
  });
  ...
  describe("enabled values", () => {
    ...
    it.each(enabledValues)(
      "handles uppercase/mixed case correctly for %s",
      (value) => {
        expect(
          shouldShowMatrixFromSearch(`?matrix=${value.toUpperCase()}`),
        ).toBe(true);
        if (value.length > 1) {
          const mixedCase =
