...
function initializeScrollSpeedWatcher(element, onChange) {
  ...
  const updateFrame = () => {
    ...
    if (newSpeed.x !== 0 || newSpeed.y !== 0) {
      ...
      // OPTIMIZATION: Only recurse if we are moving
...
setTimeout(() => {
    ...
    if (frameCount < 5) {
        console.log('SUCCESS: Frame count stayed low, loop terminated.');
    } else {
        console.log('FAILURE: Frame count is too high, loop did not terminate.');
    }
